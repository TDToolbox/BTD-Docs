...
BTD6api is coded in C++ and uses IL2CPPdumper to get access to the games files. This new method of modding allows us much more access to BTD6's code and allows for much more advanced mods. While it is much greater than previous methods of modding it is no where near the best. Modding with BTD6api is difficult because it's coded in C++ and requires you to be skilled at computer programming. Additionally in it's current state it is unreasonably resource intensive and can take several minutes to compile your mod *each time* you try to compile it. This won't remain true forever though, it's still in it's infancy stage so it will likely see significant improvement in the future. This wiki will be updated to reflect those changes
